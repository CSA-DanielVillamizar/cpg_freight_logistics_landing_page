...
public sealed class AcceptLoadCommandHandler(
    IApplicationDbContext dbContext,
    ICurrentUser currentUser,
    IDateTimeProvider clock)
    : IRequestHandler<AcceptLoadCommand, LoadSummaryResponse>
{
    public async Task<LoadSummaryResponse> Handle(
        AcceptLoadCommand request,
        CancellationToken cancellationToken)
    {
        ...
        // Domain guard: throws DomainException (-> 409) unless the load is Available.
        load.Accept(carrier.Id);
        load.AssignedCarrier = carrier;
        ...
        // SaveChanges commits the transaction (xmin optimistic concurrency check included);
        // DispatchDomainEventsInterceptor then publishes LoadAcceptedIntegrationEvent.
